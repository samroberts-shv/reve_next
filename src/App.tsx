    ...
    <>
      ...
      <div ref={bottomLeftContainerRef}>
        {activeBottomLeftMenu !== null && (
          <section
            ...
            style={{
              ...
              ...(activeBottomLeftMenu === 'adjust' && adjustButtonRef.current
                ? (() => {
                    const btnRect = adjustButtonRef.current.getBoundingClientRect()
                    const panelWidth = 300
                    return { left: Math.max(10, btnRect.left + btnRect.width / 2 - panelWidth / 2) }
                  })()
                : activeBottomLeftMenu === 'effects' && effectsButtonRef.current
                  ? (() => {
                      const btnRect = effectsButtonRef.current.getBoundingClientRect()
                      const panelWidth = selectedEffectName != null ? 300 : 320
                      return { left: Math.max(10, btnRect.left + btnRect.width / 2 - panelWidth / 2) }
                    })()
                  : {}),
        ...
        <nav className="bottom-left-actions" style={{ bottom: `${controlsBottomPx}px` }} aria-label="Canvas actions">
          <button className={`text-action-button${activeBottomLeftMenu === 'info' ? ' active' : ''}`} type="button" aria-label="Info" disabled={isMagicFixView} onClick={() => toggleBottomLeftMenu('info')}>
          ...
          <button className={`text-action-button${activeBottomLeftMenu === 'objects' ? ' active' : ''}`} type="button" aria-label="Objects" disabled={isMagicFixView} onClick={() => toggleBottomLeftMenu('objects')}>
          ...
          <button ref={adjustButtonRef} className={`text-action-button text-action-button--adjust${activeBottomLeftMenu === 'adjust' ? ' active' : ''}`} type="button" aria-label="Adjust" disabled={isMagicFixView} onClick={() => toggleBottomLeftMenu('adjust')}>
            <img className="text-action-glyph adjust-glyph" src={adjustGlyph} alt="" aria-hidden="true" />
            <span>Adjust</span>
          </button>
          <button ref={effectsButtonRef} className={`text-action-button${activeBottomLeftMenu === 'effects' ? ' active' : ''}`} type="button" aria-label="Effects" disabled={isMagicFixView} onClick={() => toggleBottomLeftMenu('effects')}>
      ...
      >
        ...
        <div className="composer-main-row">
          ...
          <input
            ...
            readOnly={isReveRendering || isMagicFixView}
            disabled={isMagicFixView}
          />
          ...
          type="button"
          aria-label="Render"
          disabled={!canRender || isMagicFixView}
          onClick={() => {
            void handleRender()
          }}
        >
